...
import { bfs, dfs, dijkstra, topologicalSort, bellmanFord, aStar, tarjan } from '../algorithms/graph';
...
const algorithms = {
  ...
  'Tarjan algorithm': { fn: tarjan, best: 'O(V + E)', avg: 'O(V + E)', worst: 'O(V + E)', space: 'O(V)' }
};
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 min-h-[400px] w-full bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden cursor-crosshair relative">
            ...
            >
              ...
              {nodes.map(node => {
                ...
                const isVisited = currentSnapshot.visitedNodes && currentSnapshot.visitedNodes.includes(node.id);
                const isActive = currentSnapshot.activeNode === node.id;
                const isSelected = selectedNode?.id === node.id;
                const isOnStack = selectedAlgo === 'Tarjan algorithm' && currentSnapshot.stack && currentSnapshot.stack.includes(node.id);
                ...
                return (
                  ...
                  >
                    ...
                    {selectedAlgo === 'Tarjan algorithm' && currentSnapshot.dfn && currentSnapshot.dfn[node.id] !== undefined && (
                      <text
                        x={node.x + 25}
                        y={node.y - 25}
                        fill="#6366f1"
                      ...
                      >
                        [{currentSnapshot.dfn[node.id]},{currentSnapshot.low[node.id]}]
                      </text>
                    )}
                  </g>
                );
        ...
        <div className="w-80 bg-white border-l border-slate-200 flex flex-col h-full shadow-md z-10">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            ...
            {/* Tarjan's Algorithm Live State */}
            {selectedAlgo === 'Tarjan algorithm' && (
              <div className="space-y-4">
                {/* DFS Stack */}
                <div className="space-y-2">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block">DFS Stack</span>
                  <div className="flex flex-wrap gap-1.5 p-2 bg-slate-50 border border-slate-200 rounded-lg min-h-[44px] items-center">
                    {(currentSnapshot.stack || []).length === 0 ? (
                      <span className="text-xs text-slate-400 italic">Empty stack</span>
                    ) : (
                      currentSnapshot.stack.map((nodeId, idx) => {
                        const n = nodes.find(node => node.id === nodeId);
                        return (
                          <span key={idx} className="bg-indigo-600 text-white px-2 py-0.5 rounded text-xs font-bold shadow-sm">
                            {n ? n.label : nodeId}
                          </span>
                        );
                      })
                ...
                <div className="space-y-2">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Strongly Connected Components (SCCs)</span>
                  <div className="space-y-2">
                    {(currentSnapshot.sccs || []).length === 0 ? (
                      <span className="text-xs text-slate-400 italic">No SCCs identified yet</span>
                    ) : (
                      currentSnapshot.sccs.map((scc, sccIdx) => (
                        <div key={sccIdx} className="bg-emerald-50 rounded-lg p-2 border border-emerald-100">
                          ...
                          <div className="flex flex-wrap gap-1 items-center">
                            {scc.map((nodeId, idx) => {
                              const n = nodes.find(node => node.id === nodeId);
                              return (
                                <span key={idx} className="bg-emerald-600 text-white px-2 py-0.5 rounded text-xs font-bold shadow-sm">
                                  {n ? n.label : nodeId}
                                </span>
                              );
                            })}
                ...
                {/* DFN and LOW values Table */}
                <div className="space-y-2">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block">DFN & LOW Tables</span>
                  <div className="border border-slate-200 rounded-lg overflow-hidden">
                    <table className="w-full text-left text-sm border-collapse">
                      <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-medium text-xs">
                        <tr>
                          <th className="p-2">Node</th>
                          <th className="p-2">DFN</th>
                          <th className="p-2">LOW</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {nodes.map(node => {
                          const dfnVal = currentSnapshot.dfn ? currentSnapshot.dfn[node.id] : undefined;
                          const lowVal = currentSnapshot.low ? currentSnapshot.low[node.id] : undefined;
                          return (
                            <tr key={node.id} className={node.id === currentSnapshot.activeNode ? "bg-amber-50" : ""}>
                              <td className="p-2 font-bold text-slate-700">
                                {node.label}
                              ...
                              <td className="p-2 font-mono text-xs">
                                {dfnVal === undefined ? '-' : dfnVal}
                              </td>
                              <td className="p-2 font-mono text-xs text-indigo-600 font-bold">
                                {lowVal === undefined ? '-' : lowVal}
                              </td>
                            </tr>
                          );
                        })}
